...
class EKWindow: UIWindow {
    ...
    init(with rootVC: UIViewController) {
        if #available(iOS 13.0, *) {
            ...
            let foregroundActiveScene = connectedScenes.first(where: { $0.activationState == .foregroundActive })
            if let scene = (foregroundActiveScene ?? connectedScenes.first) as? UIWindowScene {
